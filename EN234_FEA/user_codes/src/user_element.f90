...
subroutine user_element_static(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    else if ( element_identifier == 101 .OR. element_identifier == 111) then           ! Basic fully integrated 2D linear elastic plain strain element
    ...
    else if ( element_identifier == 102 .OR. element_identifier == 112) then           ! Basic fully integrated 2D linear elastic plain stress element

        call el_linelast_2dbasic_stress(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables

    else if ( element_identifier == 103) then           ! Basic fully integrated 2D linear elastic axisymmetic element

        call el_linelast_2dbasic_axisym(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties, element_coords, length_coord_array, &                      ! Input variables
    dof_increment, dof_total, length_dof_array, &                                                ! Input variables
    n_state_variables, initial_state_variables, &                                                ! Input variables
    updated_state_variables,element_stiffness,element_residual, fail)      ! Output variables
...
subroutine user_element_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if ( element_identifier == 1001 ) then              ! Basic fully integrated 3D linear elastic element
        ...
        call el_hyperelast_3d_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
            n_properties, element_properties,element_coords, length_coord_array, &                         ! Input variables
            dof_increment, dof_total, length_dof_array,  &                                                 ! Input variables
            n_state_variables, initial_state_variables, &                                                  ! Input variables
...
subroutine user_element_fieldvariables(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    if ( element_identifier == 1001  .OR. element_identifier == 1011 ) then              ! Basic fully integrated 3D linear elastic element
        ...
        call fieldvars_linelast_3dbasic(lmn, element_identifier, n_nodes, node_property_list, &         ! Input variables
        ...
        else if ( element_identifier == 101 .OR. element_identifier == 111 ) then          ! Basic fully integrated 2D linear elastic element
            ...
            call fieldvars_linelast_2dbasic_stress(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
            ...
            call fieldvars_linelast_2dbasic_axisym(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
                n_properties, element_properties,element_coords, length_coord_array, &                                   ! Input variables
                dof_increment, dof_total, length_dof_array, &                                                            ! Input variables
                n_state_variables, initial_state_variables,updated_state_variables, &                                    ! Input variables
                n_field_variables,field_variable_names, &                                                                ! Field variable definition
