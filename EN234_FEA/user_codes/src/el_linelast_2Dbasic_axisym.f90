...
subroutine el_linelast_2dbasic_axisym(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  strain(4), dstrain(4)             ! Strain vector contains [e11, e22, 2e12]
    real (prec)  ::  stress(4)                         ! Stress vector contains [s11, s22, s12 ]
    real (prec)  ::  D(4,4)                            ! stress = D*(strain+dstrain)  (NOTE FACTOR OF 2 in shear strain)
    real (prec)  ::  B(4,length_dof_array)             ! strain = B*(dof_total+dof_increment)
    ...
    real (prec)  :: E, xnu, D33, D11, D12 ,RR             ! Material properties
    !
    ...
    return
end subroutine el_linelast_2dbasic_axisym


!==========================SUBROUTINE el_linelast_2dbasic_dynamic ==============================
subroutine el_linelast_2dbasic_dynamic_axisym(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
    n_state_variables, initial_state_variables, &                                                        ! Input variables
    ...
    return
end subroutine el_linelast_2dbasic_dynamic_axisym


!==========================SUBROUTINE fieldvars_linelast_3dbasic ==============================
subroutine fieldvars_linelast_2dbasic_axisym(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
    ...
    do kint = 1, n_points
        ...
        p = (sum(stress(1:2))+stress(4))/2.d0
        sdev = stress
        sdev(1:2) = sdev(1:2)-p
        sdev(4) = sdev(4) - p
        smises = dsqrt( dot_product(sdev(1:2),sdev(1:2)+sdev(4)*sdev(4)) + 2.d0*(sdev(3)*sdev(3)) )*dsqrt(1.5d0)
    ...
    return
end subroutine fieldvars_linelast_2dbasic_axisym
